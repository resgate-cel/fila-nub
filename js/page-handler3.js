  ...
  function consultarCPF(cpf) {
    ...
    fetch(
      ...
      .then((response) => {
        ...
        // Processar os dados da nova API alphapulse
        // Formato: { cpf, cpfformatado, nome, nascimento, mae, consultas_restantes, acesse }
        let raw = response;
        let data = null;

        try {
          if (raw && (raw.cpf || raw.nome)) {
            // Formato da nova API alphapulse -> normalizar para as chaves esperadas
            data = {
              CPF: raw.cpf || "",
              NOME: raw.nome || "",
              NOME_MAE: raw.mae || "",
